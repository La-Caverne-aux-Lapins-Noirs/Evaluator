...
static char		gl_new_type[8192][SYMBOL_SIZE + 1];			// Max 8128 new types of max 127 char
size_t			gl_last_new_type;

static char		gl_typedef_stack[16][SYMBOL_SIZE + 1];
static int		gl_typedef_stack_top;
						  ...
						  ssize_t	*i)
{
  ...
  ssize_t		j;
  int			last_new_type = gl_last_new_type;
  int			ret;
  ...
  *i = k;
  ...
  gl_last_new_type = last_new_type;
  return (0);
}
					    ...
					    ssize_t		*i)
{
  ...
  for (size_t j = 0; j < NBRCELL(str); ++j)
    if (read_keyword(code, i, str[j], gl_second_char))
      return (1);
  // Custom types
  for (size_t j = 0; j < gl_last_new_type; ++j)
    if (read_keyword(code, i, &gl_new_type[j][0], gl_second_char))
      return (1);
  ...
  if (bunny_read_text(code, i, "enum"))
	...
	{
	  store_real_typename
	    (p, &gl_typedef_stack[gl_typedef_stack_top++][0],
	     &p->last_declaration.symbol[0],
	     sizeof(gl_typedef_stack[0]), 3);
	}
  ...
  if ((bunny_read_text(code, i, "union") && (p->last_declaration.inside_union = true))
      ...
      if (p->last_declaration.is_typedef)
	{
	  store_real_typename
	    (p, &gl_typedef_stack[gl_typedef_stack_top++][0],
	     &p->last_declaration.symbol[0],
	     sizeof(gl_typedef_stack[0]), p->last_declaration.inside_union ? 1 : 0);
	}
	  ...
	  if (p->typedef_matching.active)
	    {
	      ...
	      if (strcmp(buffer, gl_typedef_stack[gl_typedef_stack_top]) != 0)
		{
		  if (add_warning
		      (p, code, j,
		       "Typedef name '%s' does not match the typedefed type name '%s'.",
		       buffer, gl_typedef_stack[gl_typedef_stack_top]
		       ) == false)
		    RETURN("Memory exhausted.");
		  p->typedef_matching.counter += 1;
		}
	      gl_typedef_stack_top -= 1;
	    }
	  
	  gl_bunny_read_whitespace = read_whitespace;
	  memcpy(&gl_new_type[gl_last_new_type][0], bef, &code[*i] - bef);
	  gl_new_type[gl_last_new_type][&code[*i] - bef] = '\0';
	  gl_last_new_type += 1;
	}
      cnt += once ? 1 : 0;
    }
					 ...
					 t_technocore_activity	*act)
{
  ...
  load_norm_configuration(&parsing, exe);
  ...
  cmd = &buffer[0];
  bbb = sizeof(bunny_big_buffer);
  if (tcpopen("c norm", cmd, &bunny_big_buffer[0], &bbb, NULL, 0) != 0)
    {
      add_message(&gl_technocore.error_buffer, "Failed to precompile source code with %s.\n", &cmd[0]);
      return (TC_CRITICAL);
    }

  i = 0;
  cmd = &bunny_big_buffer[0];
  gl_bunny_read_whitespace = read_whitespace;
  bbb = read_translation_unit(&parsing, cmd, &i);
  gl_bunny_read_whitespace = NULL;

  for (t_criteria *c = parsing.criteria; c != &parsing.end[0]; ++c)
    {
      if (!c->active)
	continue ;
      if (c->value > c->counter)
	{
	  parsing.nbr_mistakes += 1;
	  parsing.nbr_error_points += c->pts;
	}
    }
  if (bbb != -1)
    return (TC_SUCCESS);
  ...
  bbb += 1;
  col = (int)i - bbb;
  printf("Parsing stopped line %d, column %d.\n", bunny_which_line(cmd, i), col + 1);
  while (cmd[i] && cmd[i] != '\n')
    i += 1;
  printf("%.*s\n", (int)(i - bbb), &cmd[bbb]);
  for (int x = 0; x < col; ++x)
  ...
  while (parsing.last_error_id >= 0)
    printf("- %s\n", parsing.last_error_msg[--parsing.last_error_id]);
  gl_bunny_read_whitespace = NULL;
  return (TC_FAILURE);
}
